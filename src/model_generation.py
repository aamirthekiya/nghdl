...
if port_vector_info[port_vector_count]:
    ...
    components.append(output_port[-1].split(':')[0] +
                      ": out std_logic\n\t\t\t\t")
    # if item.split(":")[1] != '1':
...
for item in input_port:
    process.append('\t\t\tobj_ref := Pack_String_To_Vhpi_String("' +
                   item.split(':')[0] + '");\n')
    process.append('\t\t\tVhpi_Get_Port_Value(obj_ref,' +
                   item.split(':')[0] + '_v,' + item.split(':')[1] + ');\n')

    if port_vector_info[port_vector_count]:
        process.append('\t\t\t' + item.split(':')[0] +
...
process.append('\t\t\treport "Iteration - "' +
...
start_server.write(
    "ghdl -a "+fname+" &&\n")
start_server.write(
    "ghdl -a "+fname.split('.')[0]+"_tb.vhdl  &&\n")
start_server.write(
    "ghdl -e -Wl,ghdlserver.o "+fname.split('.')[0]+"_tb &&\n")
start_server.write(
    "./"+fname.split('.')[0]+"_tb")
